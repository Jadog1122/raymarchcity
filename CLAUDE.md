...
- WebGL 告警先分清「提醒」和「这次绘制被拒绝」：`GL_INVALID_OPERATION: glDrawElements` = 这一趟根本没执行。bloom 因此整整一个项目没亮过。
- 一个 pass 的渲染目标纹理必须在这趟之前解绑（`tBright` 之于 bright extraction）；GL 检查绑定，不看着色器走哪个分支。
- 修好一个长期存在的 UB 之后，所有在它之上标定的阈值都失效了：回到画面重新判断，不要为了过而抬阈值。
- 启动阶段的画质自适应要按 `dt` 逐帧反应；每秒一次的调节器救不了「一帧几秒」的机器，凑齐样本之前已经冻了几分钟。
- 软件渲染（SwiftShader/llvmpipe）单独识别、钳到底、把渲染器名字留在屏幕上——够不到的机器只能靠它自己说话。
- 新加的异步依赖要问「它在隐藏标签页里还会发生吗」：rAF 在被遮挡的页面可能完全不跑，任何必须完成的流程都要有超时兜底。
...
- 墙上时间（两帧之差 dt）不是性能指标：它把「渲染慢」和「浏览器没调度我」混在一起。编译停顿/GC/窗口被遮挡都会给出巨大样本。丢弃 >1000ms 的样本，并要求连续三帧慢才降档。
- 报一个数字之前确认它测的是不是名字说的那件事：看门狗超时被记成 compileMs，屏幕上就变成「着色器难编译」——实际是 promise 饿死。
- 新断言先问「环境能不能单方面让它失败」：能就归入 FPS_SOFT 只报告（画质自适应结果和 fps、录屏同属吞吐量）。
- 实时页面上的检查不要借用定帧的阈值：相机在动，暗路段本来就不亮。实时检查只断言「渲染成立 + 交互成立」，好不好看交给那 13 张定帧。
- 科普/文档里的每个数字都要实测。首次加载的真实构成：下载（页面 74KB + three.js 265KB）、编译着色器 78ms（弱显卡几十秒）、招牌图集 24ms；骰子每帧一样贵（第 60 帧 33.27ms vs 第 460 帧 33.25ms），不是首次加载的开销。
